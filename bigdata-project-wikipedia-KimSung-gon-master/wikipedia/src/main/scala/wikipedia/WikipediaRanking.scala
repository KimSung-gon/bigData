...
class WikipediaRanking (val masterAddress: String) {
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
  	index.mapValues(iter => iter.size).collect().toList.sortBy(line => line._2)(Ordering.Int.reverse)
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
  	rdd.flatMap(article => langs.filter(lang => article.text.split(" ").contains(lang)).map(lang => (lang, 1))).reduceByKey((v1, v2) => v1 + v2).collect().toList.sortBy(line => line._2)(Ordering.Int.reverse)
  }
