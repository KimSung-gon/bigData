 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("test-case 1 scenario : two multiple five is ten?") {
	  assert(2 * 5 === 10)
  }
  test("test-case 2 scenario : List(1,2,3).head is one?") {
	  assert(List(1,2,3).head === 1)
  }
  test("test-case 3 scenario : List(1,2,3).last is three?") {
	  assert(List(1,2,3).last === 3)
  }
  test("test-case 4 scenario : two exponention five is thirty-two?") {
	  assert(Math.pow(2,5) === 32)
  }
  test("test-case 5 scenario : List(1,2,3,4,5) indexof(3) is two?") {
	  assert(List(1,2,3,4,5).indexOf(3) === 2)
  }
}
